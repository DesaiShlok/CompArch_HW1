...
# The first three functions (parse_data_files, create_enhanced_summary_plot, 
# create_violin_comparison_plot) remain unchanged from the previous version.
# I am including them here so you have the complete, runnable script.
...
def create_summary_table_html(stats_df):
    """
    Creates a styled HTML table from the summary statistics DataFrame,
    now including P95 and P99 percentiles.
    """
    # Pivot the table to a "wide" format which is better for display
    table_df = stats_df.pivot_table(
        index=['size', 'size_bytes'],
        columns='type',
        # NEW: Added p95 and p99 to the values to be aggregated
        values=['mean', 'min', 'max', 'variance', 'p95', 'p99']
    ).sort_values('size_bytes')

    # Reorder columns for better readability
    desired_order = ['mean', 'min', 'max', 'p95', 'p99', 'variance']
    col_order = [(stat, type_) for type_ in ['Ticks', 'Time (ns)'] for stat in desired_order]
    table_df = table_df[col_order]

    # Clean up the column names
    table_df.columns = [f'{stat.replace("p", "P").upper()} {type_}' for stat, type_ in table_df.columns]
    table_df = table_df.reset_index().drop(columns='size_bytes')
    table_df.rename(columns={'size': 'Data Size'}, inplace=True)
    
    # Convert the DataFrame to an HTML string with styling
    return table_df.to_html(classes='stats-table', index=False, float_format='{:.1f}'.format)
...
def main():
    ...
    # --- NEW: Calculate percentiles along with other stats ---
    def p95(x): return x.quantile(0.95)
    def p99(x): return x.quantile(0.99)
    stats_df = long_df.groupby(['size', 'size_bytes', 'type']).agg(
        mean=('value', 'mean'), min=('value', 'min'), max=('value', 'max'), 
        variance=('value', 'var'), p95=('value', p95), p99=('value', p99)
    ).reset_index()
    long_df_with_stats = pd.merge(long_df, stats_df.drop(columns=['p95', 'p99']), on=['size', 'size_bytes', 'type'])
    ...
    print("📝 Generating summary statistics table with percentiles...")
