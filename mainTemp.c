...
static inline uint64_t rdtsccpuid()
{
	unsigned long a, d;
	asm volatile("cpuid\n\t" "rdtsc\n\t": "=a"(a), "=d"(d) : "a"(0) : "rbx", "rcx");
	return a | ((uint64_t)d << 32);
...
{
	unsigned long a,d;
	// asm volatile("rdtscp\n\t" "mov %%edx,	%0\n\t" "mov %%eax,	%1\n\t" "cpuid\n\t": "=a"(a), "=d"(d) ::"%rax", "%rbx", "%rcx", "%rdx");
	asm volatile(
 	   	"rdtscp\n\t"
 	   	"mov %%edx, %0\n\t"
	   	"mov %%eax, %1\n\t"
	   	"cpuid\n\t"
	   	: "=r"(d), "=r"(a)       // outputs in any registers
    	   	:
		: "%rax", "%rbx", "%rcx", "%rdx"  // clobbered by rdtscp + cpuid
);
	return a | ((uint64_t)d << 32);
}*/
...
static inline uint64_t rdtscp()
{
    ...
    asm volatile(
        "rdtscp\n\t"
        "cpuid\n\t"
        : "=a"(a), "=d"(d)       // capture low/high TSC
        :                        // no inputs
        : "%rbx", "%rcx"          // cpuid clobbers RBX and RCX
    );
...
char *file1Ticks = "outTicksTemp.txt", *file2NSec = "outNsTemp.txt";
char *filegetTimeAnalysis="outgetTimeTemp.txt";
//@brief 
